...
Polywhaler includes simple password protection to restrict access. To enable it:

1. Set the password as a Cloudflare Workers secret:
   ```bash
   wrangler secret put APP_PASSWORD
   ```
   When prompted, enter your desired password.

2. If `APP_PASSWORD` is not set, the app will be accessible without authentication (backward compatible).
