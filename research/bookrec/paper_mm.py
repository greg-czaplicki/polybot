...
class Sim:
    ...
    def place(self, ms):
        ...
        if p is None or self.inv * p >= INV_CAP_USD:
            return
        self.order = {"price": p, "shares": QUOTE_USD / p, "q_ahead": self.bids.get(p, 0.0), "placed_ms": ms}
        self.joined += 1
...
def run_market(cid, question, t0, t1, tick, gs, sport):
    ...
    pnl_flat = pairs * 1.0 + exit_val(s0, rem0, False) + exit_val(s1, rem1, False) - cost
    pnl_mid = pairs * 1.0 + exit_val(s0, rem0, True) + exit_val(s1, rem1, True) - cost
...
print(f"paper MM: {len(res)} markets with pregame data (game_start >= {time.strftime('%Y-%m-%d %H:%M', time.gmtime(SINCE)) if SINCE else 'any'}); quote ${QUOTE_USD:.0f}/side, inv cap ${INV_CAP_USD:.0f}/token, exit T-{EXIT_MIN}m, quoting from T-{START_MIN}m")
def agg(rows, name):
    if not rows: return
    ...
    inv = sum(r["inv_end_usd"] for r in rows)
    print(f"  {name:8s} mkts={len(rows):3d} fills={fills:4d} filled=${f:8.0f} pairs=${pairs:6.0f} unpaired-at-exit=${inv:6.0f} | P&L flatten ${pf:+7.2f} ({(pf/f*100 if f else 0):+.2f}c/$) mid ${pm:+7.2f} ({(pm/f*100 if f else 0):+.2f}c/$) | fills/quoting-hour {fills/(qm/60) if qm else 0:.1f}  $filled/quoting-hour ${f/(qm/60) if qm else 0:.0f}")
agg(res, "ALL")
by = defaultdict(list)
