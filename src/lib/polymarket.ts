...
/**
 * Fetches volume data for a market by conditionId
 */
export async function fetchMarketVolume(
  ...
  try {
    ...
    if (!response.ok) {
      // Don't throw - volume data is optional
      if (response.status === 404) {
        return null
      }
      console.warn(`Volume request failed (${response.status}) for conditionId: ${conditionId}`)
      return null
    }

    const data = (await response.json()) as MarketVolume
    return data
  } catch (error) {
    if (signal?.aborted) {
      return null
...
/**
 * Fetches open interest data for a market by conditionId
 */
export async function fetchMarketOpenInterest(
  conditionId: string,
  signal?: AbortSignal,
): Promise<MarketOpenInterest | null> {
  if (!conditionId) {
    return null
  }
  ...
  try {
    ...
    if (!response.ok) {
      // Don't throw - OI data is optional
      if (response.status === 404) {
        return null
      }
      console.warn(`OI request failed (${response.status}) for conditionId: ${conditionId}`)
      return null
    }

    const data = (await response.json()) as MarketOpenInterest
    return data
  } catch (error) {
    if (signal?.aborted) {
      return null
...
/**
 * Fetches both volume and open interest for a market
 */
export async function fetchMarketMetrics(
  ...
  const [volume, openInterest] = await Promise.all([
    fetchMarketVolume(conditionId, signal),
    fetchMarketOpenInterest(conditionId, signal),
  ])
